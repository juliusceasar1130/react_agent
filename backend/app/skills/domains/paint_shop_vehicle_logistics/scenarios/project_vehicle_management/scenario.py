...
DIRECT_QUERY_META = {
    ...
    "parameters": {
        ...
        "project_vehicle_no_filter": {
            ...
            "sql_fragment": 'AND project_vehicle_no LIKE CONCAT(\'%\', :project_vehicle_no_filter, \'%\')',
            "template_sql_fragments": {
                "current_positions": 'AND fpc."project_vehicle_no" LIKE CONCAT(\'%\', :project_vehicle_no_filter, \'%\')',
                "orders_overview": 'AND pvo."project_vehicle_no" LIKE CONCAT(\'%\', :project_vehicle_no_filter, \'%\')',
                "quality_defects": 'AND vde."project_vehicle_no" LIKE CONCAT(\'%\', :project_vehicle_no_filter, \'%\')',
            },
        },
        "body_type_filter": {
            "type": "string",
            "description": "按车型/车身类型模糊匹配",
            "required": False,
            "example_values": [],
            "usage": "替换 {body_type_filter} 占位符。",
            "widget": "input",
            "source_table": "fct.fct_vehicle_position_current",
            "source_column": "body_type",
            "sql_fragment": 'AND body_type LIKE CONCAT(\'%\', :body_type_filter, \'%\')',
            "template_sql_fragments": {
                "current_positions": 'AND fpc."body_type" LIKE CONCAT(\'%\', :body_type_filter, \'%\')',
                "quality_defects": 'AND vde."body_type" LIKE CONCAT(\'%\', :body_type_filter, \'%\')',
            },
        },
        "process_area_filter": {
            "type": "string",
        ...
        "has_defect_only_filter": {
            ...
            "options": [
                {"value": "", "label": "全部"},
                {"value": "true", "label": "仅缺陷车"},
            ],
            "usage": "用于所有模板。替换 {has_defect_only_filter} 占位符。",
            "sql_fragment": 'AND EXISTS (SELECT 1 FROM fct.fct_vehicle_defect_enriched _vde WHERE _vde.project_vehicle_no = project_vehicle_no AND _vde.total_defect_count > 0)',
            "template_sql_fragments": {
                "current_positions": 'AND EXISTS (SELECT 1 FROM fct.fct_vehicle_defect_enriched _vde WHERE _vde.project_vehicle_no = fpc.project_vehicle_no AND _vde.total_defect_count > 0)',
                "orders_overview": 'AND EXISTS (SELECT 1 FROM fct.fct_vehicle_defect_enriched _vde WHERE _vde.project_vehicle_no = pvo.project_vehicle_no AND _vde.total_defect_count > 0)',
                "quality_defects": 'AND vde."total_defect_count" > 0',
            },
        },
    },
}
