...
SCENARIO = {
    ...
    "workflow": [
        "确认用户提供了明确的时间段（start_time 与 end_time）。如果提问模糊（如'昨天'），按标准时间转换公式计算边界。",
        "将 `ods.carbody_history` 限制在 `L3ACC21IS01`, `L3ACC21IS02`, `L3ACC21IS03` 读写站，代表车辆已到达检测入口。",
        "与 `fct.fct_vehicle_defect_detection` 进行 LEFT JOIN 连接车身ID，过滤检测主键为 NULL 的漏检车身。",
        "关联 `dim.dim_vehicle_profile` 获取车型名称 and 位置信息，按过车时间 `pass_time` 升序排列输出列表。"
    ],
    "rules": [
