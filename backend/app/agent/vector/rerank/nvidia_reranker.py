# backend/app/agent/vector/rerank/nvidia_reranker.py
"""
...
class NvidiaReranker(BaseReranker):
    ...
    def __init__(
        ...
        logger.info(
            f"NvidiaReranker 初始化: model={model}, "
    ...
    def rerank(
        ...
        if not query or not query.strip():
            logger.warning("Reranker: 查询为空，跳过重排序")
        ...
        if not valid_indices:
            logger.warning("Reranker: 所有文档文本为空，跳过重排序")
            return [ScoredDocument(document=doc, score=0.0) for doc in documents]
        ...
        try:
            ...
            logger.warning(
                "Reranker API 超时 (%ss)，降级使用原始排序", self.timeout
            )
            return [ScoredDocument(document=doc, score=0.0) for doc in documents]
        except requests.exceptions.RequestException as e:
            logger.warning("Reranker API 请求失败，降级使用原始排序: %s", e)
            return [ScoredDocument(document=doc, score=0.0) for doc in documents]
        except Exception as e:
            logger.error("Reranker 解析响应失败，降级使用原始排序: %s", e)
            return [ScoredDocument(document=doc, score=0.0) for doc in documents]

        # 解析响应并排序
        rankings = result.get("rankings", [])
        if not rankings:
            logger.warning("Reranker API 返回空 rankings，降级使用原始排序")
            return [ScoredDocument(document=doc, score=0.0) for doc in documents]
        ...
        for i, item in enumerate(ranked_results):
            ...
            logger.info("  Reranker #%d: score=%.4f, term='%s'", i + 1, score, term)
        ...
        if self.score_threshold is not None:
            ...
            logger.info(
                "Reranker 阈值过滤: threshold=%s, 过滤前=%d, 过滤后=%d",
        ...
        if self.top_n is not None and len(ranked_results) > self.top_n:
            ...
            logger.info("Reranker Top-N 截断: 保留前 %d 条", self.top_n)

        logger.info("Reranker 完成: 最终保留 %d 条文档", len(ranked_results))
