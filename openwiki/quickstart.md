...
- **[Architecture](architecture/overview.md)** — the runtime topology, dual persistence modes, and the agent assembly / state / subagent / middleware / tool subsystems.
...
- **Docs-only intent reports** (`docs/deepagent/`, `docs/multiagent_sidechannel/`, `docs/llamaindex_rag/`) — link-only design intent; the wiki references them from the relevant runtime pages rather than restating them. Revisit as a summary page only if a specific design decision becomes load-bearing.
