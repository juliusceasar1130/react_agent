...
* **实现方式**：
  在代码的 SQL 查询拦截器（如包装工具 `sql_db_query`）中，**不再一刀切地根据行数拦截**。而是引入一套“智能旁路机制”：通过 AST 解析或简单的表名匹配，判定本次 SQL 查询的目标表是否为系统注册的**维度表/字典表**（如包含 `dim_` 前缀，或通过白名单配置的表如 `process_areas`、`car_models`、`colors`）。
* **截断策略**：
  - 如果仅包含维度表，自动提升硬限制（如提高到 500 行，甚至完全放行）。
  - 如果包含事实表，则严格执行 30 行截断。
* **优点**：改动小，对大模型完全透明，保留了 SQL 查询的灵活性，且完美解决截断痛点。
* **缺点**：如果大模型写了多表 `JOIN`（维度表与事实表混查），需要稍微复杂的逻辑来判定是否为纯维度表查询。
...
def _is_pure_dimension_query(query: str) -> bool:
    """
    判断当前查询是否仅涉及维度表/字典表。
    通过简单的正则检测 SQL 中的 FROM 和 JOIN 子句。
    """
    # 转换为小写并清理换行
    q = query.lower()
    
    # 获取白名单中的维度表
    dim_tables = settings.dimension_tables
    
    # 检查是否包含任何事实表关键字。如果在 FROM/JOIN 中包含了非白名单的表名，则不属于纯维度查询
    # (更严谨的做法是解析表名，此处为轻量方案)
    # 如果 SQL 包含了我们已知的事实表（例如 vehicle_tracking、defect_logs），则返回 False
    fact_indicators = ["vehicle_tracking", "defect_logs", "historical_trace"] # 系统内的事实表
    if any(fact in q for fact in fact_indicators):
        return False
        
    # 如果 SQL 中出现了白名单内的维度表，并且没有出现事实表
    if any(dim in q for dim in dim_tables):
        return True
