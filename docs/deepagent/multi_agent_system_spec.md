...
- **子智能体 (sql_subgraph) Middleware 管道（领域专属与上下文隔离）**：
  - `BusinessRagMiddleware`（项目自研）：包含业务术语、同义词与三层数据库物理词典向量检索（自适应 Task 描述并执行物理词典自愈）。
  - `SkillMiddleware`（项目自研）：动态挂载场景技能 Prompt。
  - `PromptCompilerMiddleware`（项目自研）：合并表 DDL、字段注释、数据库词典与全量 SQL 工具历史消息裁剪/折叠至子 System Prompt。
