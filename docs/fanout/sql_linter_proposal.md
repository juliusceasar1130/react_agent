...
|---|---|---|---|---|
| `SEM-001` | JOIN 关联列唯一性校验 | 提取多表 `JOIN ON` 关联列集合。满足任一条件即判定安全，否则拦截：<br>1. **物理主/唯一键**：关联列集是对应表唯一/主键列集的超集。<br>2. **子查询 GROUP BY/DISTINCT**：右侧子查询的分组或 DISTINCT 列覆盖关联列。<br>3. **极值子查询过滤**：通过 `MAX(col)`/`MIN(col)` 过滤特定列并关联外层键。<br>4. **ROW_NUMBER 窗口过滤**：子查询含 `ROW_NUMBER()` 且外层 ON 过滤为 `= 1`。<br>5. **LIMIT 1 全局去重**：右侧子查询带有 `LIMIT 1`。<br>6. **CTE 去重白名单**：右侧为 `WITH` 临时表时，递归检测其内部是否包含 `GROUP BY`、`DISTINCT`、窗口函数去重 `ROW_NUMBER() + WHERE rn = 1`，或引用的链式 CTE 去重属性。 | **1. 复合主键支持**：如上进行超集包含比对。<br>**2. 无聚合函数降级**：若整个 SQL 中无 SUM/AVG/COUNT，降级为 `WARNING`。<br>**3. 旁路豁免**：检测到行首有 `-- linter-bypass: SEM-001` 强制放行。<br>**4. CTE 粒度级联传播**：支持多层嵌套 WITH 子查询的去重特征自动传递。 | **ERROR** |
